                            ...
                            NimfServiceIC *target)
{
  ...
  /* Check if we should hide candidates - be more conservative for librime 1.13+ */
                                       ...
                                       NimfServiceIC *target)
{
  ...
  /* Check if we should clear preedit - be more conservative for librime 1.13+ */
  gboolean should_clear = FALSE;
  
#if HAS_NEW_API
  /* For librime 1.13+: Only clear when there's truly nothing to show */
  should_clear = (context.composition.length == 0 && 
                  context.menu.num_candidates == 0 && 
                  !context.commit_text_preview);
#else
  /* For older librime: Use original logic */
