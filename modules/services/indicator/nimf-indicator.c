...
static GMenu * nimf_indicator_build_section1 (NimfIndicator *indicator, NimfServer *server) {
  ...
  for (i = 0; engine_ids && engine_ids[i]; i++) {
    ...
    path = g_build_filename (NIMF_MODULE_DIR, engine_ids[i], NULL); // Updated function
...
static void nimf_indicator_create_appindicator (NimfIndicator *indicator) {
  ...
  if ((!gnome_xkb_is_available () || !gnome_is_running ()) && !g_strcmp0 (g_getenv ("XDG_SESSION_TYPE"), "x11")) {
    XklConfigRec *rec;
    GSettings    *settings;
