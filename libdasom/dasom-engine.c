                                      ...
                                      const gchar **strv)
{
  ...
  dasom_candidate_update_window (engine->priv->caller->daemon->candidate,
                                 strv);
}
...
dasom_engine_show_candidate_window (DasomEngine *engine)
{
  ...
  dasom_candidate_show_window (engine->priv->caller->daemon->candidate);
}
...
dasom_engine_hide_candidate_window (DasomEngine *engine)
{
  ...
  dasom_candidate_hide_window (engine->priv->caller->daemon->candidate);
}

void
dasom_engine_select_previous_candidate (DasomEngine *engine)
{
  g_debug (G_STRLOC ": %s", G_STRFUNC);

  dasom_candidate_select_previous_item (engine->priv->caller->daemon->candidate);
}

void
dasom_engine_select_next_candidate (DasomEngine *engine)
{
  g_debug (G_STRLOC ": %s", G_STRFUNC);

  dasom_candidate_select_next_item (engine->priv->caller->daemon->candidate);
}
