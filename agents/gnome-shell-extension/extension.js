...
let nimf_agent, nimf_menu, extension;
...
const NimfMenu = new Lang.Class({
  Name: 'NimfMenu',
  Extends: PanelMenu.Button,

  _init: function() {
    this.parent(0.0, _("Nimf"));

    this._hbox = new St.BoxLayout({ style_class: 'panel-status-menu-box' });
    this._label = new St.Label({ text: _("Nimf"),
                                 y_expand: true,
...
function enable()
{
  ...
  nimf_menu = new NimfMenu;

  Main.panel.addToStatusArea('nimf-agent-extension', nimf_menu, 0, 'right');

  nimf_agent = new Nimf.Agent;

  nimf_agent.connect('engine-changed', function(agent, text) {
    child = nimf_menu._hbox.get_child_at_index (0);

    if (text == 'focus-out')
    {
      if (child != nimf_menu._icon)
        nimf_menu._hbox.replace_child (child, nimf_menu._icon);
    }
    else
    {
      if (nimf_menu._label.text != text)
        nimf_menu._label.text = text;

      if (child != nimf_menu._label)
        nimf_menu._hbox.replace_child (child, nimf_menu._label);
    }
  });

  nimf_agent.connect('disconnected', function(agent) {
    child = nimf_menu._hbox.get_child_at_index (0);
    nimf_menu._hbox.replace_child (child, nimf_menu._warning_icon);
  });

  nimf_agent.connect_to_server();
}
